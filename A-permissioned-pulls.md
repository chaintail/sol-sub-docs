# Permissioned Pulls: Solana's New Authorization Layer for Subscriptions and Allowances
...
2026-06-10 SDK ground-truth corrections (verified against @solana/subscriptions@0.3.0 dist/index.d.ts;
see research-digest.md "CORRECTIONS" section for line-level evidence):
  1. SA PDA seeds CORRECTED: ["SubscriptionAuthority", user, tokenMint] (literal CamelCase
     SUBSCRIPTION_AUTHORITY_SEED), one PDA per (user, mint) — the 2026-06-09 audit item 2 below
     "resolved" this the WRONG way (delegator-only seeds came from the digest, which the SDK
     contradicts). Body + diagram fixed.
  2. Plan size CORRECTED: 491 bytes exactly (PLAN_SIZE = 491), was "~528". Body, diagram, and
     build-walkthrough fixed.
  3. Hour-granularity tradeoff RESCOPED: hours (1..8760) is a plan-layer constraint; direct
     recurring delegations take periodLengthS in SECONDS — sub-hourly metering is possible.
     Anatomy table + tradeoffs section fixed.
  4. Token-2022 claims CORRECTED: "incl. confidential transfers" removed; replaced with the
     SDK's rejected-extension error table (118-124: confidential transfer, non-transferable,
     permanent delegate, transfer hook, transfer fee, mint close authority, pausable). The
     docs-vs-SDK discrepancy is noted in-body, framed as evidence not gotcha.
  5. Pullers/destinations tradeoff RESCOPED: pullers ARE rotatable via update_plan (UpdatePlanData
     has a pullers field); only destinations force plan sunset. Treasury-rotation point retained
     via destinations.

Carried over from the 2026-06-09 fix pass (still standing):
  - USDC-decimals example rephrased to digest wording; state-update-before-CPI described neutrally;
    CADD-on-Solana = planned, not live; card pre-auth analogy explicitly limited.

Remaining least-certain claims:
  1. The "12 gates" framing: split of "period rollover + cap" into two steps is logically faithful
     but exact step boundaries in program source may differ.
  2. The configured-vs-inert TransferHook distinction (an inert hook passing) comes from the docs;
     the SDK error constant MINT_HAS_TRANSFER_HOOK (121) does not itself encode that nuance.
-->


<!-- CADD-FLIP CONTINGENCY (apply only if CADD launches on Solana before publish): change the CADD sentence near the Canadian close from planned/not-live to live (cite the launch source); strip "day it lands"-style language; add source. -->
