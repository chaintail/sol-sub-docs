# Canada's Stablecoin Subscription Layer: CADD, Solana Allowances, and the CAD-Native Billing Rail Nobody Has Built Yet
...
<!-- ACCURACY SELF-AUDIT (updated 2026-06-10 SDK ground-truth pass; previous pass 2026-06-09)
[VERIFY] tags in this memo: 0.

2026-06-10 SDK ground-truth corrections (verified against @solana/subscriptions@0.3.0
dist/index.d.ts; line-level evidence in research-digest.md "CORRECTIONS"):
1. Section 2 table token-support row CORRECTED: "including confidential transfers" was FALSE
   per the SDK (error MINT_HAS_CONFIDENTIAL_TRANSFER = 118); now "restricted extension set."
2. Section 3 recurring-delegation description CORRECTED: delegation periods are in SECONDS
   (periodLengthS), not hours; the hours (1..8760) granularity belongs to merchant plans only.
   This honestly STRENGTHENS the metered-billing angle — noted in 4b (sub-hourly metering under
   a client-held cap).
3. Archetype 4b reworded from "730-hour period" to a seconds-exact 30-day window + added the
   metered-billing upside.
4. Section 5 compliance paragraph EXPANDED: the rejection list is 7 extensions (errors 118-124:
   confidential transfer, non-transferable, permanent delegate, transfer hook, transfer fee,
   mint close authority, pausable), not hooks alone; the docs-vs-SDK confidential-transfer
   discrepancy is flagged in-body, sided with the SDK. The CADD design-question framing gets
   NARROWER (skip-the-hook is not sufficient), which the text now says.
5. Treasury-rotation wording re-checked: section 6's "destination accounts are immutable, so
   treasury rotation means sunsetting a plan" is CONFIRMED correct per UpdatePlanData (no
   destinations field; pullers field present — puller rotation does NOT force sunset, and the
   memo never claimed it did).

Carried over from the 2026-06-09 fix pass — all 5 previous tags resolved by softening to durable wording:
1. PAD/card-rail age — RESOLVED: "roughly four decades" → "decades-old payment rails"; no precise-age claim remains.
2. Card processing rate — RESOLVED: exact "2.9% + 30¢" dropped in favor of "fees that commonly run near three percent plus a fixed per-transaction fee"; the per-renewal arithmetic is now explicitly illustrative ("on the order of a dollar and change of every $40 renewal").
3. National Bank "Schedule I" — RESOLVED: characterized as "one of Canada's largest banks"; no Schedule I assertion remains.
4. CRA stablecoin tax — RESOLVED: framed purely as an open question for a tax professional; no stated tax treatment remains.
5. Stripe-origins close — RESOLVED: cut entirely.

Also resolved in the fix pass:
- CADD-on-Solana status verified 2026-06-09: NOT live (most recent authoritative statement, May 22 2026, says "plans to expand to Solana"; zero on-chain/explorer/DEX evidence). The memo's "planned, not live" framing throughout (table, section 5) is correct and stands.
- Chargeback claim reworded: program transfers are not card-network chargebacks; refund and dispute policy moves up a layer, to the merchant — no "chargebacks disappear" category claim or 90-day-window figure remains.
- Compliance-hook section reframed as an OPEN DESIGN QUESTION for the CADD team ("what will CADD's Solana mint configuration be?"), not a predicted incompatibility.
- Staffing/timeline color ("two or three engineers ship in a season") cut; the business gap itself stays.
- "Anyone who tells you the CAD-native version is live is selling something" cut.

Remaining least-certain claim:
1. The CADD compliance-extension question (section 5) — the SDK establishes the 7-extension
   rejection list and CADD is regulated; whether CADD's Solana deployment would use any rejected
   extension is unknown (no public information on CADD's Solana token design exists in the
   digest). The memo frames this strictly as an open design question, but it remains the most
   original and therefore least-corroborated argument in the piece.

Landmine compliance check (all 9): (1) called a standalone program, never a Token-2022 extension ✓; (2) no SIMD/protocol-change claim ✓; (3) auditor stated as Cantina ✓; (4) no-scheduler stated repeatedly, off-chain trigger explicit ✓; (5) native SOL exclusion stated with wSOL workaround ✓; (6) u64::MAX approval framed as gated by per-authorization caps ✓; (7) immutability stated precisely (core terms immutable; status/end_ts/pullers/metadata updatable) ✓; (8) framed as authorization-not-execution; Clockwork named as dead, not "replaced" ✓; (9) "multi-delegator" name not used ✓.
-->

<!-- CADD-FLIP CONTINGENCY (apply only if CADD launches on Solana before publish):
- Section 2 table: move "CADD on Solana" to LIVE TODAY (cite the launch source); update the CADD row detail accordingly.
- Archetype 4a: premise changes — CAD-denominated plans become available; the USDC-FX "catch" paragraph becomes optional/historical.
- Sections 5/6 reframe: CADD is live; the remaining unknown = token configuration, especially whether the Solana mint uses any of the 7 SDK-rejected extensions (errors 118-124) + who integrates first.
- Swap the "build on USDC now; wait for CADD" framing → "build the scheduler layer; settle in USDC or CADD per mint compatibility".
-->
